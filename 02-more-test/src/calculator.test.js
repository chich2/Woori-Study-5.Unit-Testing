...
describe('2. 유효하지 않은 입력값', () => {
    test.skip('2-1. 하나라도 공백 값이 전달될 경우, NaN을 반환한다.', () => {
        // Arrange
        const numbers = [1, '']; // '' = 공백값
    ...
    test.skip('2-2. 문자열로 작성된 숫자값이 전달되어도 연산 결과를 반환한다.', () => {
        const numbers = ['1', '2'];
    ...
    test.skip('2-3. 하나라도 숫자가 아닌 값이 전달될 경우, NaN을 반환한다.', () => {
        const result = add([1,'string']);
        expect(result).toBeNaN();
    });

    test.skip('2-4. 빈 배열이 전달될 경우 0을 반환한다.', () => {
        const numbers = [];
...
test('add()에 인수가 전달되지 않을 경우, 예외를 던진다', () => {
    const resultFunction = () => {
        add();
    }

    // expect(add()).toThrow(); // 진짜 에러가 발생해버림
